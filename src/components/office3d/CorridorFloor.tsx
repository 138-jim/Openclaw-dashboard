...
export default function CorridorFloor() {
  return (
    <group>
      {CORRIDORS.map((c, i) => (
        <group key={i}>
          {/* Corridor base — box geometry eliminates z-fighting */}
          <mesh position={[c.x, -0.04, c.z]} receiveShadow>
            <boxGeometry args={[c.w, 0.04, c.d]} />
            <meshToonMaterial color={COLOR_A} gradientMap={grad} />
          </mesh>
          {c.tiles.map((tile, ti) => (
            <mesh key={ti} position={[tile.x, -0.018, tile.z]}>
              <boxGeometry args={[tile.s - 0.02, 0.005, tile.s - 0.02]} />
