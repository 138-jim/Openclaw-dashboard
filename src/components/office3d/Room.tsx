...
export default function Room({ roomIndex, label, name, emoji, glowColor, isError }: RoomProps) {
  ...
  return (
    <group position={[origin.x, 0, origin.z]}>
      {/* ── Floor (box, not plane — prevents z-fighting) ── */}
      <mesh position={[ROOM_W / 2, -0.025, ROOM_H / 2]} receiveShadow>
        <boxGeometry args={[ROOM_W - 0.3, 0.05, ROOM_H - 0.3]} />
        <meshToonMaterial color="#C8B08A" gradientMap={grad} />
      </mesh>

      {/* ── Rug ── */}
      <mesh position={[ROOM_W / 2, 0.01, ROOM_H * 0.45]} receiveShadow>
        <boxGeometry args={[4.5, 0.02, 3]} />
      ...
      {/* ── Window on back wall — inset into wall so no z-fighting ── */}
      <mesh position={[ROOM_W / 2, WALL_HEIGHT * 0.6, wallThick / 2]}>
        <boxGeometry args={[1.8, 1.0, wallThick + 0.02]} />
      ...
      {/* ── Error overlay ── */}
      {isError && (
        <mesh ref={errorRef} position={[ROOM_W / 2, 0.01, ROOM_H / 2]}>
          <boxGeometry args={[ROOM_W - 0.5, 0.02, ROOM_H - 0.5]} />
          <meshStandardMaterial color="#ef4444" emissive="#ef4444" emissiveIntensity={0.5} transparent opacity={0.15} />
        </mesh>
      )}
    </group>
  );
