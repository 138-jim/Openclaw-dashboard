...
export const ROOM_H = 6.5;
export const CORRIDOR = 1.5;
export const COLS = 5;
export const ROWS = 4;
export const WALL_HEIGHT = 2;

export const GRID_W = COLS * ROOM_W + (COLS - 1) * CORRIDOR; // 63
export const GRID_H = ROWS * ROOM_H + (ROWS - 1) * CORRIDOR; // 46

// Break room to the right of the grid
export const BREAK_ROOM_W = 8;
export const BREAK_ROOM_X = GRID_W + CORRIDOR;
export const W = BREAK_ROOM_X + BREAK_ROOM_W;

// Visitor lobby in front of the office (below grid)
export const LOBBY_H = 6;
export const LOBBY_Z = GRID_H + CORRIDOR;
export const H = LOBBY_Z + LOBBY_H;
...
export function getDeskPos3D(roomIndex: number): { x: number; z: number } {
  ...
  return { x: o.x + ROOM_W * 0.45, z: o.z + ROOM_H * 0.4 };
}

export function getChairPos3D(roomIndex: number): { x: number; z: number } {
  const o = getRoomOrigin3D(roomIndex);
  return { x: o.x + ROOM_W * 0.5, z: o.z + ROOM_H * 0.55 };
}

// Door position — placed IN the corridor (0.5 units past the wall)
// so agents are fully clear of the doorframe
export function getDoorPos3D(roomIndex: number): { x: number; z: number } {
  const o = getRoomOrigin3D(roomIndex);
  return { x: o.x + ROOM_W / 2, z: o.z + ROOM_H + 0.5 };
}

// Interior door position — just inside the room near the door
export function getInsideDoorPos3D(roomIndex: number): { x: number; z: number } {
  const o = getRoomOrigin3D(roomIndex);
  return { x: o.x + ROOM_W / 2, z: o.z + ROOM_H - 0.5 };
}
...
// Break room seats — distributed in the right-side break room area
const BRX = BREAK_ROOM_X; // break room left edge X
// Generate seats in a grid with good spacing so agents don't overlap
const BREAK_ROOM_SEATS_3D: { x: number; z: number }[] = Array.from({ length: 20 }, (_, i) => {
  const col = i % 4;
  const row = Math.floor(i / 4);
  return {
    x: BRX + 1 + col * 1.8,
    z: 1.5 + row * (GRID_H - 3) / 5,
  };
});
...
// Visitor lobby seats — in front of the office
const LOBBY_SEATS: { x: number; z: number }[] = Array.from({ length: 10 }, (_, i) => ({
  x: 3 + (i % 5) * (GRID_W - 6) / 4,
  z: LOBBY_Z + 1.5 + Math.floor(i / 5) * 3,
}));

export function getLobbySeat3D(index: number): { x: number; z: number } {
  return LOBBY_SEATS[index % LOBBY_SEATS.length];
}

// Horizontal corridor center Z between row r and row r+1
export function hCorridorZ(row: number): number {
  return (row + 1) * ROOM_H + row * CORRIDOR + CORRIDOR / 2;
}

// Vertical corridor center X between col c and col c+1
export function vCorridorX(col: number): number {
